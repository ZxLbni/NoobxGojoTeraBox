...
"""
    ...
    await m.reply(
        ...
        buttons=[
    [
        Button.url("Channel ", url="https://t.me/GojoTera"),
    ],
    [
...
async def broadcast_message(m: UpdateNewMessage):
    ...
    all_users = await bot.get_participants(-1002340771449)  # Replace with your group ID
...
# ┃
# ┃ To check availabe plan do /plan in chat group @GojoxChat
# ╚═════════════════⍟
# For subscription inquiries, contact @noob_je.
# """

#     # Send the welcome message
#     check_if = await is_user_on_chat(bot, "@GojoTera", m.peer_id)
#     if not check_if:
#         return await m.reply("Please join @GojoTera then send me the link again.")
...
async def handle_message(m: Message):
    ...
    if not check_if:
        return await m.reply("Please join @GojoTera then send me the link again.")
    check_if = await is_user_on_chat(bot, "@GojoxChat", m.peer_id)
    if not check_if:
        return await m.reply(
            "Please join @GojoxChat then send me the link again."
        )
